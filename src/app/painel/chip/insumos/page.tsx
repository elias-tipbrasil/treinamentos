...
export default async function ChipInsumosPage() {
  const user = await getCurrentUser();
  if (!user) redirect("/login");
  const produtos = await getChipProdutos(["insumo"]);
  ...
  return (
    <section className="max-w-7xl mx-auto w-full px-6 py-10">
      ...
      <ChipTabs active="insumos" />
      <EstoqueManager produtos={produtos} grupo="insumo" />
    </section>
  );
}
